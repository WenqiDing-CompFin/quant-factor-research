# Generated Results

All committed results in this directory come from the deterministic synthetic
demo and are not evidence of real-market alpha.

- `metrics.csv`: portfolio summary statistics.
- `monthly_returns.csv`: signal-date portfolio, benchmark, cost, and turnover data.
- `factor_ic.csv`: monthly single-factor Rank IC observations.
- `factor_ic_summary.csv`: mean IC, IC dispersion, positive ratio, and ICIR.
- `factor_quantile_returns.csv`: monthly next-period returns for five factor buckets.
- `factor_quantile_summary.csv`: average bucket returns and top-minus-bottom spread.
- `cost_sensitivity.csv`: results under multiple linear cost assumptions.
- `equity_curve.png`: growth of one synthetic dollar versus the equal-weight universe.
- `run_manifest.json`: data provenance, fixed factor definitions, portfolio rule,
  and transaction-cost assumptions for the run.
- `public_factors/official_current_snapshot.csv`: real factor returns reported by
  the Kenneth French Data Library for May 2026, including trailing windows and
  the official source URL.
- `public_factors/latest/`: committed derived evidence from the full official
  histories: factor summaries, correlations, fixed-period momentum-decile
  diagnostics, charts, source URLs, and archive SHA-256 hashes. Raw archives and
  optional monthly exports remain outside Git.

The public snapshot is aggregate real-market evidence and is not generated by
the synthetic stock-selection pipeline. Run `python run_public_factor_validation.py`
to download and analyze the full official monthly histories.

Regenerate all files from the repository root with:

```bash
python run_research.py
```
